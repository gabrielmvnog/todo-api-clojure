...
(defproject todo-api "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "todo-api.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.6.1"]]}
             :uberjar {:aot [todo-api.server]}}
  :main ^{:skip-aot true} todo-api.server)
